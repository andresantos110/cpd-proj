...
void simulate(char* front_grid, char* back_grid, Species* record, int num_generations, long long x_side, long long slice_side, int pid, int worldsize) {
    ...
    for (long long x = 1; x < x_side - 1; x++) {
    ...
    for (int i = 0; i < num_generations; i++) {
        ...
        MPI_Sendrecv(&front_grid[slice_area], slice_area, MPI_CHAR, neighbours[0], pid, &front_grid[(x_side - 1) * slice_area], slice_area, MPI_CHAR, neighbours[1], neighbours[1],
                      MPI_COMM_WORLD, &status[0]);
        MPI_Sendrecv(&front_grid[(x_side - 2) * slice_area], slice_area, MPI_CHAR, neighbours[1], pid, &front_grid[0],
                      slice_area, MPI_CHAR, neighbours[0], neighbours[0], MPI_COMM_WORLD, &status[1]);
